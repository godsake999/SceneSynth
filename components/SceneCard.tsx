    ...
    <div className="flex flex-col bg-slate-800 rounded-2xl shadow-2xl border border-slate-700 transition-all duration-300">
      ...
      <div className="relative aspect-[9/16] bg-slate-900 rounded-t-2xl overflow-hidden group">
        ...
        <div className="absolute top-4 left-4 bg-black/60 backdrop-blur-md text-white text-[10px] font-bold px-2.5 py-1.5 rounded-lg border border-white/10 flex items-center gap-2 shadow-lg">
          SCENE {scene.id}
          {hasImage && (
            <span className={`text-[8px] px-1.5 py-0.5 rounded-md font-black uppercase tracking-tighter ${scene.imageSource === 'gemini' ? 'bg-blue-500/80' : scene.imageSource === 'imagefx' ? 'bg-green-500/80' : 'bg-purple-500/80'} text-white`}>
              {scene.imageSource === 'gemini' ? 'Gemini' : scene.imageSource === 'imagefx' ? 'ImageFX' : 'Flux'}
            </span>
          )}
        </div>
      ...
      <div className="p-4 flex flex-col gap-4 bg-slate-800 rounded-b-2xl border-t border-slate-700">
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            className={`flex flex-col items-center justify-center rounded-xl text-[9px] font-black tracking-tighter transition-all duration-200 h-12 ${hasImage
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex-1 flex flex-col items-center justify-center rounded-xl text-[9px] font-black tracking-tighter transition-all duration-200 h-12 ${hasAudio
                ? 'bg-slate-700/50 text-slate-400 hover:bg-slate-700 hover:text-white border border-slate-600/50'
            ...
            >
              <div className="flex items-center gap-1.5">
                ...
                {isLoadingAudio ? "Syncing..." : (hasAudio ? "Redo" : "Voice It")}
              </div>
            ...
            {hasAudio && !isLoadingAudio && (
              <button
                onClick={() => {
                  console.log('Playing audio from URL:', scene.audioUrl);
                ...
                className="w-12 h-12 flex items-center justify-center rounded-xl bg-blue-600/80 hover:bg-blue-500 text-white transition-all duration-200 shadow-lg"
                title="Play Audio"
              >
