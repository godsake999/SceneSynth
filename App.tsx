...
const createDefaultScenes = (count: number = 5): Scene[] => Array.from({ length: count }, (_, i) => ({
...
const App: React.FC = () => {
  ...
  const [sceneCount, setSceneCount] = useState(5); // Dynamic scene count (3-10)
  ...
  const handleNewStory = () => {
    if (window.confirm("Start a new story? This will clear all current progress and fields.")) {
      ...
      setScenes(createDefaultScenes(5));
      setIsPlanning(false);
  ...
  const toggleJsonEditor = () => {
    if (!showJson) {
      ...
      try {
        ...
        if (data.scenes) {
          const importedSceneCount = data.scenes.length;
          setSceneCount(importedSceneCount);
          setScenes(data.scenes.map((s: any, i: number) => ({
            ...createDefaultScenes(importedSceneCount)[i],
            ...s,
  ...
  const handleGenerateOutroImage = async () => {
    ...
    try {
      const contextImage = scenes[scenes.length - 1]?.imageUrl || scenes[scenes.length - 2]?.imageUrl || intro.imageUrl;
  ...
  const handleGenerateAllText = async () => {
    ...
    setError(null);
    setIsPlanning(true);
    setScenes(createDefaultScenes(sceneCount)); // Reset scenes to match count
    try {
      const plan = await generateStoryPlan(topic, style, sceneCount);
  ...
  const downloadAllImages = async () => {
    const imagesToDownload = [
      ...
      { url: outro.imageUrl, name: (scenes.length + 2).toString() }
    ].filter(img => !!img.url);
          ...
          <div className="bg-slate-800/50 p-6 rounded-2xl border border-slate-700 mb-10">
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
              ...
              <div className="md:col-span-1">
                <label className="text-xs font-bold text-slate-400 uppercase ml-1 mb-2 block">Scenes</label>
                <div className="flex items-center gap-2 bg-slate-900 border border-slate-700 rounded-xl px-3 py-2.5">
                  <input
                    ...
                    max="10"
                    value={sceneCount}
                    onChange={e => setSceneCount(Math.min(10, Math.max(3, parseInt(e.target.value) || 5)))}
            ...
            <div className="flex flex-col bg-slate-800 rounded-2xl shadow-2xl border border-indigo-500/20 overflow-hidden relative transition-all duration-300">
              <div className="relative aspect-[9/16] bg-slate-900 group">
                ...
                <div className="absolute top-4 left-4 bg-black/60 backdrop-blur-md text-white text-[10px] font-bold px-2.5 py-1.5 rounded-lg border border-white/10 flex items-center gap-2 shadow-lg">
                  INTRO
                  {intro.imageUrl && (
                    <span className={`text-[8px] px-1.5 py-0.5 rounded-md font-black uppercase tracking-tighter ${intro.imageSource === 'gemini' ? 'bg-blue-500/80' : intro.imageSource === 'imagefx' ? 'bg-green-500/80' : 'bg-purple-500/80'} text-white`}>
                      {intro.imageSource === 'gemini' ? 'Gemini' : intro.imageSource === 'imagefx' ? 'ImageFX' : 'Flux'}
                    </span>
                  )}
                </div>
            ...
            <div className="flex flex-col bg-slate-800 rounded-2xl shadow-2xl border border-emerald-500/20 overflow-hidden relative transition-all duration-300">
              <div className="relative aspect-[9/16] bg-slate-900 group">
                ...
                <div className="absolute top-4 left-4 bg-black/60 backdrop-blur-md text-white text-[10px] font-bold px-2.5 py-1.5 rounded-lg border border-white/10 flex items-center gap-2 shadow-lg">
                  OUTRO
                  {outro.imageUrl && (
                    <span className={`text-[8px] px-1.5 py-0.5 rounded-md font-black uppercase tracking-tighter ${outro.imageSource === 'gemini' ? 'bg-blue-500/80' : outro.imageSource === 'imagefx' ? 'bg-green-500/80' : 'bg-purple-500/80'} text-white`}>
                      {outro.imageSource === 'gemini' ? 'Gemini' : outro.imageSource === 'imagefx' ? 'ImageFX' : 'Flux'}
                    </span>
                  )}
                </div>
